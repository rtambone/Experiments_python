# -*- coding: utf-8 -*-
"""
Created on Fri Feb  5 10:52:42 2021
...
for day in range(0,len(data)):
    ...
    for TrialType in range(1,8):
        ...
        if TrialType==1: 
            ax.flat[day+1].plot(t, R, color= colors[0], linewidth=2, label='CS1+')
        elif TrialType==2:
            ax.flat[day+1].plot(t, R, '--', color= colors[0], linewidth=2, label='CS1')
        elif TrialType==3: 
            ax.flat[day+1].plot(t, R, color= colors[1], linewidth=2, label='CS2+')
        elif TrialType==4: 
            ax.flat[day+1].plot(t, R, '--', color= colors[1], linewidth=2, label='CS2')
        elif TrialType==5: 
            ax.flat[day+1].plot(t, R, color= colors[2], linewidth=2, label='NS1')
        elif TrialType==6: 
            ax.flat[day+1].plot(t, R, color= colors[3], linewidth=2, label='NS2')
        elif TrialType==7: 
            ax.flat[day+1].plot(t, R, color= colors[4], linewidth=2, label='US')
        ax.flat[day+1].legend()
        ax.flat[day+1].set_title('Day '+str(day+2))
        ax.flat[day+1].axvline(color='black')
        ax.flat[day+1].set_xlabel('Time (s)')
        ax.flat[day+1].set_ylabel('Hz')
    ax.flat[day+1].add_patch(Rectangle((1,ax.flat[day+1].get_ylim()[0]),
                                     1,ax.flat[day+1].get_ylim()[1] + abs(ax.flat[day+1].get_ylim()[0]), 
                                     alpha=0.5))