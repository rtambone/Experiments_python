# -*- coding: utf-8 -*-
"""
Created on Sun May 16 10:22:58 2021
...
trial_types= data[int(day)]['SessionData']['TrialTypes'][0:nTrials]
trials_data= data[int(day)]['SessionData']['RawEvents']['Trial'][0:nTrials]  # a list with every trial
...
for TrialType in range(1,3):
    ...
    if TrialType==1:    # valve click no odor
        ax.flat[0].plot(t, R, color= colors[2], linewidth=1, label='CS-')
    elif TrialType==2:
        ax.flat[0].plot(t, R, color= colors[4], linewidth=1, label='CS+')
    ax.flat[0].legend()
    ax.flat[0].set_title('')
...
#fig.suptitle(mouse_name)
ax.flat[1].plot(x_axes, cum_alicksOdor_convolved[:plot_trials], 'b', label='Cymene')
ax.flat[1].plot(x_axes, cum_alicksNoOdor_convolved[:plot_trials], 'r', label='Carvone')
