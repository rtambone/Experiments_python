# -*- coding: utf-8 -*-
"""
Created on Tue May 18 15:10:11 2021
...
for TrialType in range(1,3):
    ...
    if TrialType==1:    # valve click no odor
        ax.flat[0].plot(t, R, color= colors[4], linewidth=1.5, label='No Odor')
    elif TrialType==2:
        ax.flat[0].plot(t, R, color= colors[2], linewidth=1.5, label='Cymene')
    ax.flat[0].legend()
    ax.flat[0].set_title('')
...
ax.flat[2].set_title('No odor \n trials with aLicks = ' + str(round(percentCsMinus[-1],2))+'%')
