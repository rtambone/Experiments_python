# -*- coding: utf-8 -*-
"""
Created on Sun May  9 19:25:32 2021
...
for TrialType in range(1,4):
    ...
    for Trial in range(0, idxTrialType.size):
        if 'Port1In' in trials_data[idxTrialType[0][Trial]]['Events']:    #if there was a PortIn 
           licks_ts= trials_data[idxTrialType[0][Trial]]['Events']['Port1In'] - 5   # centered on valve opening
           licksXtrial.append(licks_ts)
    [R,t]= get_psth(licksXtrial, sig=0.1, time_windows= [-5, 25])
    if TrialType==1:    # valve click no odor
        ax.plot(t, R, color= colors[1], linewidth=2, label='No odor')
    elif TrialType==2:
        ax.plot(t, R, color= colors[3], linewidth=2, label='CS+')
    elif TrialType==3: 
        ax.plot(t, R, '--', color= colors[3], linewidth=2, label='CS+ no reward')
    ax.legend()
...
ax.add_patch(Rectangle((0,ax.get_ylim()[0]),
                        2,ax.get_ylim()[1] + abs(ax.get_ylim()[0]), 
                        alpha=0.5))
